...
\label{Temperature_dependency}
To quantify the temperature dependency of observables, the measurements have been performed with some considerations in mind.
...
To perform sweep over the temperature grid, the system will start at the hot state, and gradually decrease in temperature.
...
This choice has been made deliberately, due to the large correlation time of specifically the Metropolis algorithm at lower temperatures, which would require larger $N_{Burn}$ to reach thermal equilibrium, especially when starting from a hot state.
...
The entirety of the measurement comprises a total $N_{Rep}=20$ temperature sweeps for all grid sizes in $L={8, 16, 32, 64, 128, 256}$.
\begin{figure}[h]
    \includegraphics[width=0.4275\textwidth]{../figs/Metropolis_M.pdf}
    \includegraphics[width=0.4275\textwidth]{../figs/Metropolis_X.pdf}
    \caption{\label{fig:Metropolis_Observables}Temperature dependency of $\langle M \rangle$ and $\chi$, generated using the Metropolis algorithm.}
\end{figure}
\par
...
\begin{figure}[h]
    ...
    \caption{\label{fig:Wolff_Observables}Temperature dependency of $\langle M \rangle$ and $\chi$, generated using the Wolff algorithm.}
\end{figure}
\par
...
The same behavior can be seen for this case as well.
...
\begin{figure}
    ...
    \caption{\label{fig:Autocorrelation_Time_Metropolis}Autocorrelation times $\tau_{\text{exp}}$ as a function of temperature $T$ for the Metropolis algorithm for different lattice sizes.}
\end{figure}
\par
...
\begin{figure}
    ...
    \caption{\label{fig:TC_fit}Plotted are the temperatures at which $\chi$ peaks for various lattice sizes. Fitted is a line according to eq.~(\ref{eq:bkt_scaling}).}
\end{figure}
\par
Fig.~\ref{fig:TC_fit} displays the peak temperatures $T_{C, \chi} (L)$ with a line fit using $T_c(L) = T_c(\infty) + \text{A}\cdot \ln(L)^{-1/\nu}$.
The fitted parameters are found to be $T_c(\infty) = 0.884 \pm 0.026$ and $\nu = 0.562 \pm 0.057$.
...
\subsection{XY model on a 3D lattice}
The XY model is not necessarily restricted to a two dimensional lattice.
In order to apply it to a 3d lattice, or potentially even higher dimensions, the Metropolis and Wolff algorithm, and the extraction of the energy $E$ and magnetization $M$ have to be adjusted to account for the newly added neighbors.
After the adjustment, the measurement can be performed in the same manner as for the 2D case.
The analysis will also be analogous, and the relevant plots can be found in Appendix~\ref{fig:3DWolff_Energy_SpecificHeat}13.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.4275\textwidth]{../figs/3DWolff_X.pdf}
    \caption{\label{fig:3DWolff_x}Measured susceptibility on the 3D lattice using the Wolff algorithm.}
\end{figure}
\par
Fig.~\ref{fig:3DWolff_x} displays the susceptibility on the 3D lattice for lattice sizes of $L={6,7,8,9,10,11,12}$, generated using the Wolff algorithm.
Due to the increased computation times in the higher dimension, larger lattices have not been simulated.
A similar behavior for $\chi$ can be observed as for the 2D case, with an apparent critical temperature at around $T_c \approx 2.2$, which is significantly higher than the critical temperature for the 2D case.
However, the smaller lattice sizes made it difficult to define a clear peak, and as such the method of fitting a line through the curve did not yield meaningful results.
But, it is still possible to perform the data collapse method.
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.4275\textwidth]{../figs/3DData_Collapse_X.pdf}
    \caption{\label{fig:3DData_Collapse_X}Rescaled values of the susceptibility $\chi(L,t)\cdot L^{-\frac{\gamma}{\nu}}$ plotted against $L\cdot \exp (-b \cdot t^{-\nu})$ simulated using Wolff. $T_c$, $\nu$, $\gamma$ and $b$ are estimated by eye to achieve the best collapse.}
\end{figure}
\par
Fig.~\ref{fig:3DData_Collapse_X} illustrates the data collapse for the 3D case, using visually estimated parameters.
The collapse was acceptable for values around $T_c = 2.2 \pm 0.1$, $\nu = 0.195 \pm 0.01$, $\gamma = 0.4 \pm 0.01$ and $b = 2.4 \pm 0.2$.
The current leading estimate for the inverse critical temperature is $\beta=0.454165$, which corresponds to $T_c=2.201843$, which matches our estimation~\cite{Massimo2001}.
However, the estimation for $\nu$ and $\gamma$ are deviating a lot from the literature values of $\nu = 0.67155$ and $\gamma = 1.3177$ \cite{Massimo2001}.
Interestingly enough, their ratio $\gamma / \nu = 2.0513 \pm 0.5235$ does match within the error boundary.